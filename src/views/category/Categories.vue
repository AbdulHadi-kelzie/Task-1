    ...
    <div class="col-12 lg:col-12 xl:col-12">
        <div class="card">
            <DataTable :value="items" showGridlines :rows="10" dataKey="id" :rowHover="true" filterDisplay="menu" :loading="loading" responsiveLayout="scroll" lazy paginator @sort="sort($event)" sortMode="multiple" :multiSortMeta="sortFields">
                ...
                <Column sortable field="name" :header="$t('field.categoryName')">
                    <template #body="{ data }">
                        {{ data.name }}
                    </template>
                    <!--<template #filter="{ filterModel }">
                        <InputText v-model="filterModel.value" @keydown.enter="filter('name', filterModel)" type="text" class="p-column-filter" :placeholder="$t('field.search')" />
                    </template>-->
                </Column>
